...
void zone_up(ClickRecognizerRef recognizer, void *context) {
	if(tZone<24)
		tZone++;
	persist_write_int(NUM_TZONE_KEY, tZone);
...
void showEditTimeZone()
{
	...
	Layer *root_layer = window_get_root_layer(setZoneW);
	...
	layer_add_child(root_layer, text_layer_get_layer(setZoneW_zone));
	...
	layer_add_child(root_layer, text_layer_get_layer(setZoneW_label));
	...
	layer_add_child(root_layer, text_layer_get_layer(setZoneW_disclaim));
	*/
