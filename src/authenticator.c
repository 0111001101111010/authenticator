...
// shamelessly stolen from WhyIsThisOpen's Unix Time source: http://forums.getpebble.com/discussion/4324/watch-face-unix-time
	...
	unix_time = ((0-tZone+current_time->tm_isdst)*3600) + /* time zone offset */          /* 0-tZone+current_time.tm_isdst if it ever starts working. */
...
void handle_init(void) {
	...
	if(persist_exists(NUM_TZONE_KEY)){
		tZone = persist_read_int(NUM_TZONE_KEY);
	}
	else {
	...
	layer_add_child(root_layer, text_layer_get_layer(label));
	layer_add_child(root_layer, text_layer_get_layer(token));
	layer_add_child(root_layer, text_layer_get_layer(ticker));
